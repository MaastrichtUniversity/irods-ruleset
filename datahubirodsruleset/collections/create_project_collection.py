...
def create_project_collection(ctx, project_id, title):
    """
    Create a new project collection at the input project id
    ...
    project_id : str
        To which project the project collection will be created
    title : str
        The project collection title to add as an AVU

    Returns
    -------
    str
        The newly project collection id
    """
    retry = 0
    error = -1
    ...
    while error < 0 and retry < 10:
        latest_project_collection_number = ctx.callback.getCollectionAVU(
            project_path, ProjectAVUs.LATEST_PROJECT_COLLECTION_NUMBER.value, "", "", TRUE_AS_STRING
        )["arguments"][2]
    ...
    if error < 0 and retry >= 10:
        msg = f"ERROR: Collection '{title}' attempt no. {retry} : Unable to create {new_project_collection_path}"

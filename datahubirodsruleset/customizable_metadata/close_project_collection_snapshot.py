...
def close_project_collection_snapshot(ctx, project_id, collection_id, username):
    """
    This rule closes a project collection root, the schema.json , instance.json and the .metadata_versions sub-folder
    for both rods and the specified user.  It only closes the .metadata_versions recursively
    ...
    try:
        ...
        ctx.callback.msiWriteRodsLog(
            f"DEBUG: '{metadata_folder_path}' doesn't exist yet while setting ACL for '{username}' in close_project_collection_snapshot",
            0,
        )

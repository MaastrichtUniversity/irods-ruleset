...
class TestPolicies:
    ...
    def test_pre_proc_for_coll_create_first(self):
        ...
        create_coll_when_ingesting = "export clientUserName={} && imkdir /nlmumc/ingest/direct/{}/foobar".format(self.manager1, self.token)
        with pytest.raises(subprocess.CalledProcessError) as e_info:
    ...
    def test_pre_proc_for_coll_create_second(self):
        ...
        metadata_versions = "export clientUserName={} && imkdir /nlmumc/ingest/direct/{}/.metadata_versions".format(self.manager1, self.token)
        with pytest.raises(subprocess.CalledProcessError) as e_info:
    ...
    def test_pre_proc_for_data_obj_open(self):
        ...
        put_file_on_tape = "export clientUserName={0} && iput -fR arcRescSURF01 {1} /nlmumc/home/{0}/instance.json".format(self.manager1, TMP_INSTANCE_PATH)
        ...
        subprocess.check_call("export clientUserName={0} && irm -rf /nlmumc/home/{0}/instance.json".format(self.manager1), shell=True)
    ...
    def test_set_resc_scheme_for_create_first(self):
        ...
        subprocess.check_call(create_collection, shell=True)
        get_instance()
        put_instance = "export clientUserName={} && iput {} {}/instance.json".format(self.manager1, TMP_INSTANCE_PATH, collection_path)
        subprocess.check_call(put_instance, shell=True)
    ...
    def test_set_resc_scheme_for_create_second(self):
        ...
        get_instance()
        put_instance = "export clientUserName={} && iput {} /nlmumc/projects/{}/instance.json".format(self.manager1, TMP_INSTANCE_PATH, self.project_id)
        with pytest.raises(subprocess.CalledProcessError) as e_info:
            subprocess.check_call(put_instance, shell=True)
    ...
    def test_set_resc_scheme_for_create_third(self):
        ...
        get_instance()
        put_instance = "export clientUserName={user} && iput {instance} /nlmumc/ingest/direct/{{path}}instance_test_3.json".format(user=self.manager1, instance=TMP_INSTANCE_PATH)
        with pytest.raises(subprocess.CalledProcessError) as e_info:
        ...
        output_ils = subprocess.check_output("ils -l /nlmumc/ingest/direct/{}/instance_test_3.json".format(self.token), shell=True)
    ...
    def test_set_resc_scheme_for_create_fourth(self):
        ...
        put_instance = "export clientUserName={user} && iput {instance} /nlmumc/ingest/zones/{{path}}instance_test_3.json".format(user=self.manager1, instance=TMP_INSTANCE_PATH)
        with pytest.raises(subprocess.CalledProcessError) as e_info:
            subprocess.check_call(put_instance.format(path=""), shell=True)
        subprocess.check_call(put_instance.format(path=token+"/"), shell=True)
        output_ils = subprocess.check_output("ils -l /nlmumc/ingest/zones/{}/instance_test_3.json".format(token), shell=True)
        assert "stagingResc01" in output_ils
    ...
    def test_set_resc_scheme_for_create_fifth(self):
        ...
        set_dropzone_state = "imeta set -C /nlmumc/ingest/direct/{token} state {{state}}".format(token=self.token)
        subprocess.check_call(set_dropzone_state.format(state="ingesting"), shell=True)
        put_instance = "export clientUserName={} && iput {} /nlmumc/ingest/direct/{}/instance_test_3.json".format(self.manager1, TMP_INSTANCE_PATH, self.token)
        with pytest.raises(subprocess.CalledProcessError) as e_info:
            subprocess.check_call(put_instance, shell=True)
        subprocess.check_call(set_dropzone_state.format(state="open"), shell=True)

    ...
    for participant_name in participant_names:
        if users[participant_name] == "rodsuser":
            user_information = json.loads(
                ctx.callback.get_user_metadata(participant_name, "")["arguments"][1]
            )
            rule_result[participant_name] = {
                "display_name": user_information["displayName"]
                if user_information["displayName"]
                else participant_name,
                "email": user_information["email"],
            }
        elif users[participant_name] == "rodsgroup":
            group_display_name = json.loads(
                ctx.get_user_attribute_value(
                    participant_name, "displayName", FALSE_AS_STRING, "result"
                )["arguments"][3]
            )
            rule_result[participant_name] = {
                "display_name": group_display_name["value"]
                if "value" in group_display_name
                else participant_name
            }
            for query_result in row_iterator(
                "USER_NAME",
                "USER_GROUP_NAME = '{}'".format(participant_name),
                AS_LIST,
                ctx.callback,
            ):
                if query_result[0] not in rule_result:
                    user_information = json.loads(
                        ctx.callback.get_user_metadata(query_result[0], "")[
                            "arguments"
                        ][1]
                    )

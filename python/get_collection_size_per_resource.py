...
def get_collection_size_per_resource(ctx, project):
    ...
    for row in row_iterator("COLL_NAME, META_COLL_ATTR_NAME, META_COLL_ATTR_VALUE",
                            "META_COLL_ATTR_NAME like 'dcat:byteSize' AND COLL_NAME like '{}'".format(
                                project_path),
                            AS_LIST,
                            ctx.callback):
        total_sizes[row[0]] = row[2]
    ...
    for row in row_iterator("COLL_NAME, META_COLL_ATTR_NAME, META_COLL_ATTR_VALUE",
                            ...
                            AS_LIST,
                            ctx.callback):
        relative_size = (float(row[2]) / float(total_sizes[row[0]])) * 100
        result = dict(collection=row[0], resourceAttr=row[1][19:], size=row[2], relative_size = round(relative_size, 1))

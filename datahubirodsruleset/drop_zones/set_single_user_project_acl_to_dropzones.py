...
from datahubirodsruleset.core import make, Output, FALSE_AS_STRING, format_project_path


@make(inputs=[0, 1], outputs=[], handler=Output.STORE)
def set_single_user_project_acl_to_dropzones(ctx, project_id, username):
    """
    This rule transfers the ACLs of the input user that exist on a project level to all of its drop-zones

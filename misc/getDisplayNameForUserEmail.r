...
IRULE_getDisplayNameForUserEmail(*email,*result) {
    ...
    *userDisplayName = ""
     ...
     # First try to find an irods user with a username equal to the email address provided
     foreach (*Row in SELECT USER_NAME WHERE USER_NAME == "*email"  ) {
                 *userName = *Row.USER_NAME
     }
     # If the user_name does not exist try to find a user with a matching email address
     if (*userName == "") {
              ...
              if (*userName == "") {
                               failmsg(-1, "ERROR: A user with email adress '*email' was not found!");
              }
      }
     # Get the isdplay name givenb the username

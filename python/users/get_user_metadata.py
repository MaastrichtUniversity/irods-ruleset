...
def get_user_metadata(ctx, username):
    ...
    ret = ctx.get_username_attribute_value(result["username"], "email", "true", "result")["arguments"][3]
    result["email"] = json.loads(ret)["value"]

    ret = ctx.get_username_attribute_value(result["username"], "displayName", "true", "result")["arguments"][3]

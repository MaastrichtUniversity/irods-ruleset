"""
/rules/tests/run_test.sh -r get_collection_size -a "/nlmumc/projects/P000000010,B,none" -j
"""
from datahubirodsruleset.core import make, Output, FALSE_AS_STRING


@make(inputs=[0, 1, 2], outputs=[3], handler=Output.STORE)
def get_collection_size(ctx, project_collection_path, unit, rounding_type):
    """
    ...
    size_bytes = float(ctx.callback.getCollectionAVU(
        project_collection_path, "dcat:byteSize", "", "0", FALSE_AS_STRING
    )["arguments"][2])
